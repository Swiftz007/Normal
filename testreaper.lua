...
local function removeESP(char)
    ...
    if hrp then
        local h = hrp:FindFirstChild("HealthESP")
...
local function createHealthESP(char)
    ...
    local gui = Instance.new("BillboardGui")
    gui.Name = "HealthESP"
    gui.Size = UDim2.new(0, 6, 0, 40)
    gui.StudsOffset = Vector3.new(-1.6, 1, 0)
    gui.AlwaysOnTop = true
    gui.Adornee = hrp
...
function applyESP(char)
    ...
    if HealthESPEnabled then
        createHealthESP(char)
    end
end

------------------------------------------------
-- REFRESH ALL PLAYERS (สำคัญ)
------------------------------------------------
function refreshAll()
...
------------------------------------------------
-- PLAYER HOOK (NO LEAK / NO DUPLICATE)
------------------------------------------------
