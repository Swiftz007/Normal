...
--=====================================================
-- 🔥 OPTIMIZED MOVEMENT ENGINE (REPLACED & FIXED)
--=====================================================
local State = {
    WS = false,
    JP = false,
...
-- [ HUMANOID CACHE FUNCTION ]
local function GetHum()
    local char = LP.Character
    if not char then return nil end
    if currentHum and currentHum.Parent == char then return currentHum end
...
local function HookChar(char)
    ...
    task.wait(0.5) -- รอให้ Engine เกมโหลดค่าคงที่ของแมพนั้นๆ ให้เสร็จ
    if not initialized or DefaultWS == nil then
...
RunService.Heartbeat:Connect(function()
    local hum = GetHum()
    if not hum or not initialized then return end

    -- --- WalkSpeed Logic ---
    local targetWS = State.WS and WSValue or DefaultWS
    if hum.WalkSpeed ~= targetWS then
        hum.WalkSpeed = targetWS
    end

    -- --- JumpPower Logic ---
    local targetJP = State.JP and JPValue or DefaultJP
    if State.JP then
        if not hum.UseJumpPower then hum.UseJumpPower = true end
        if hum.JumpPower ~= targetJP then
            hum.JumpPower = targetJP -- แก้จาก .Power เป็น .JumpPower
        end
    elseif hum.JumpPower ~= DefaultJP then
